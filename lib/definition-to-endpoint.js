...
function definitionToEndpoint({ method, url }, definition) {
  ...
  const acceptHeader = headerParameters.find(
  ...
  const endpoint = {
    ...
    responses,
    changes: (definition["x-githubBreakingChanges"] || []).map((change) => {
      const type = change.type.toUpperCase();
      const defaults = parameters[change.after.name];
      const before = toParamter(type, change.before, defaults);
      const after = toParamter(type, change.after, defaults);
